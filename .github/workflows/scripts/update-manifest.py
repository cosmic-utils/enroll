...
def update_manifest(filepath: str, tag: str, commit: str) -> None:
    ...
    if count == 0:
        ...
        print(
            "Regex pattern match failed, trying fallback line-by-line replacement...",
            file=sys.stderr,
        )
    ...
    if count > 0:
        ...
        print(
            f"Updated manifest file {filepath} to tag '{tag}' and commit '{commit}' successfully!"
        )
    else:
        print(
            "Error: Could not find/update the git source in manifest!", file=sys.stderr
        )
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "Usage: python3 update-manifest.py <manifest_file> <tag> <commit>",
            file=sys.stderr,
        )
        sys.exit(1)
