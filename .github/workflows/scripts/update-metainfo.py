...
def parse_changelog(filepath):
    ...
    matches = list(re.finditer(r"^##\s+\[([^\]]+)\](?:\s*-\s*([0-9-]{10}))?", content, re.MULTILINE))
    if not matches:
...
def update_metainfo(xml_path, version, date, items):
    ...
    release_xml += '        <ul>\n'
    for item in items:
        escaped_item = item.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;').replace('"', '&quot;')
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python3 update-metainfo.py <changelog_path> <xml_path>", file=sys.stderr)
        sys.exit(1)
